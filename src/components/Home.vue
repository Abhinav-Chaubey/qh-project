  ...
  <v-container style="border: 1px solid #80808030;border-radius:5px; border-bottom:none" class="mt-5 pl-0 pr-0 pt-0">
    ...
    <v-row style="padding: 0 12px;">
      <v-col cols="11" style="border-right: 1px solid #80808030;border-bottom: 1px solid #80808030;">
        <v-row>
      <!-- First Column: Upload Banner Image -->
      <v-col cols="4" style="border-right: 1px solid #80808030;" class="text-left">
        <span class="am-table-sub-title-small-grey">Upload Banner Image</span>
      </v-col>
  ...
  <v-col cols="1" style="border-bottom: 1px solid #80808030;">
    ...
    <v-row v-for="(item, key) in contents" :key="key" style="padding: 0 12px;">
      <v-col cols="11" style="border-right: 1px solid #80808030;border-bottom: 1px solid rgba(128, 128, 128, 0.19);" :style="contents && contents.length -1 === key ? 'background-color:#80808024' : ''">
        <v-row>
      <!-- First Column: Upload Banner Image -->
      <v-col cols="4" style="border-right: 1px solid #80808030;">
        <div class="mb-2 text-left">
          <span class="am-banner-title">Banner Image</span>
        </div>
        <div v-if="!item.bannerImageData" class="d-flex align-center mb-5" style="height: 60px; border: 1px dashed #737070; border-radius:4px; position: relative;background-color:#80808054;">
          <!-- Drop area -->
          <input type="file" name="candidateUploader" ref="candidateUploader"
            ...
            @change="handleFileUpload($event.target.files, key, 'banner')"
            />
        ...
        <div v-if="item.bannerImageData" class="d-flex">
          <img :src="item.bannerImageData" style="max-width: 150px; max-height: 150px; border-radius: 4px;">
          <div class="text-center margin--20">
            <span class="remove1" @click="removeimage(key, 'banner')">&minus;</span>
          </div>
        </div>
      </v-col>

      <!-- Second Column: Text Fields -->
      <v-col cols="4"  class="pb-7" style="border-right: 1px solid #80808030;">
